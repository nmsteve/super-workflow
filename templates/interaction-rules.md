...
- **Investigation answers stay technical and short:** when the user asks what caused something, or asks to see a specific artefact such as an error body or an endpoint, answer exactly that and stop. Give the evidence, not narration: method and full URL called, request payload, verbatim response status and JSON body, matching log lines, and `file:line` for the code that produced it. Mark anything inferred from reading code rather than captured from a real run as inferred. No fixes, options, next steps, or Results/Considerations/Action needed sections unless asked; no listing of what was healthy or unaffected. Aim for under ten lines. Never write to shared state to gather evidence: replay only the call expected to fail, never a control call designed to succeed, and ask first if proving the cause requires a write.
