...
- **Pull request base enforcement:** before opening a pull request, verify that its destination is the repository's recorded long-lived base branch, such as `main`, `master`, or `development`. For a normal repository, open the PR against the base branch on `origin`. For a forked repository, open exactly one PR, against the corresponding base branch on `upstream`, and publish the work branch to whichever remote that PR requires. Never open a PR against a feature, task, or dependency branch.
