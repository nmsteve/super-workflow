...
- **Session rename automation:** after creating and publishing an issue branch, rename the session to the exact branch name as a normal implementation step and continue without pausing for user confirmation. In Claude Code, run `claude-rename-session <exact-branch-name>`; in Codex, run `codex-rename-session <exact-branch-name>`. The Claude Code helper updates the session name on disk, so the current terminal tab title only catches up after a restart. If the helper is unavailable or fails, report the concrete error and continue without blocking implementation; do not change the established branch naming convention.
