...
- **Session rename gate:** after creating and publishing an issue branch, stop before implementation and say, “Copy and run the command below to rename this session.” Show `/rename <exact-branch-name>` on its own line, display the prompt only once, and pause for exactly one user response. Because the interface may consume the rename command without exposing it to the assistant, treat the next user response—such as `go ahead`, `done`, `continue`, `skip`, or new direction—as resolving the gate and process that response normally. Never repeat the rename instruction, demand proof that renaming succeeded, execute the rename automatically, or change the established branch naming convention.
